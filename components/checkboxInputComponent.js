...
function CheckboxInputComponent({label, setInfo, data, stateName, currentValue}) {
    const [selectedValue, setSelectedValue] = useState({});
    const [selection, setSelection] = useState('');
    const handleChange = async (v) => {
        v = v.replace(/ /g, '')
        setSelectedValue(state => ({...state, [v]: !state[v]}))
    }
    ...
    const submit = () => {
        if (stateName === 'requestType') {
            ...
            } else if (selectedValue['SimpleDelivery']) {

                setInfo(stateName, 'simple')
                setSelection('simple')
            } else if (selectedValue['AdvancedDelivery']) {

                setInfo(stateName, 'advance')
                setSelection('advance')
            } else {
                setInfo(stateName, '')
                setSelection('')
            }
            ...
            setSelection(selectedData.join(','))
            setInfo(stateName, selectedData)
        }
        handleClose()
    }
    ...
    useEffect(() => {
        if (stateName === 'requestType') {
            if (currentValue === 'simple_and_advance') {
                ...
                setSelectedValue({AdvancedDelivery: true})
                setInfo(stateName, 'advance')
                setSelection('advance')
            } else {
                setInfo(stateName, '')
    ...
    return (
        <div>
            <ListItem
                sx={{borderRadius: '10px', width: '100%', background: '#F8F8F8'}}
            ...
            >
                <ListItemButton role={undefined} dense>
                    <ListItemText sx={{color: '#9E9E9E'}} primary={selection ? selection : label}/>
                </ListItemButton>
            </ListItem>


            <Dialog disableEscapeKeyDown open={open} onClose={handleClose} fullWidth>
                <DialogTitle align={'center'} display='flex' justifyContent='center'>{label}

                    <IconButton onClick={handleClose} sx={{mt: '-27px', mr: '-24px'}}>
                            ...
                            {data?.map((v, i) => {
                                ...
                                return <ListItem
                                    ...
                                    secondaryAction={
                                        <IconButton edge="end" aria-label="comments">
                                            <Checkbox
                                                checked={selectedValue ? !!selectedValue[v.replace(/ /g, '')] : false}
                                                value={v}/>
                                        </IconButton>
                                    }
                                    disablePadding
