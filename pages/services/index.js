...
function Index(props) {
    ...
    // console.log({filters, cities, transportation_types})
    useEffect(() => {
        if (!token) {
            Router.push('/')
        } else
        ...
        <Box>
            {isLoading || loading ? <LinearProgress/> :
                <>
                    ...
                    <Box marginX='6.94%' marginTop='30px' marginBottom='40px' height='83vh'>
                        <Box marginBottom='20px' height='6.46vh'>
                            <Box display='flex' padding='.8%' fontWeight='600' borderRadius='10px' fontSize='24px'
                                 bgcolor="#fff">
                                <Box>Cloud Request</Box>
                                <Box>
                                    <span style={{
                                        marginLeft: '10px',
                                        marginTop: '2px'
                                ...
                                <Box>
                                    <Button variant='contained' onClick={() => setSelectedFilter(null)}>Back</Button>
                                </Box>
                            </Box>
